...
let owner = $('#owner');
let getOnwerButton = $('#getOnwerButton');
let transferOwnerTo = $('#transferOwnerTo');
let transferOwnerButton = $('#transferOwnerButton');

let transferTo = $('#transferTo');
let transferToValue = $('#transferToValue');
let transferToButton = $('#transferToButton');
...
// 確認 Owner
getOnwerButton.on('click', function () {
	//確認 Owner
	$.get('/owner', {
		address: bankAddress,
	}, function (result) {
		if (result.owner) {
			log(result.owner, '確認 Owner')
			owner.text('Owner帳戶: ' + result.owner)
		}
		else {
			log(result)
		}
